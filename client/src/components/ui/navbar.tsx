  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 w-full z-50
      ...
      <div className="absolute top-0 left-0 w-full h-0.5 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-purple-600/0 via-purple-600/80 to-purple-600/0"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex justify-between items-center">
          ...
          <Link href="/" className="flex items-center space-x-3 group">
            ...
            <div className="w-12 h-12 relative">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0,0,256,256" className="w-full h-full">
                ...
                <g fill="#a855f7" fillRule="nonzero" stroke="none" strokeWidth="1" strokeLinecap="butt" strokeLinejoin="miter" strokeMiterlimit="10" strokeDasharray="" strokeDashoffset="0" fontFamily="none" fontWeight="none" fontSize="none" textAnchor="none" style={{mixBlendMode: "normal"}} filter="url(#glow)">
            ...
            <div className="flex flex-col">
              <span className="text-xl font-bold tracking-wider">
                ...
                <span className="bg-gradient-to-r from-purple-400 to-purple-600 bg-clip-text text-transparent font-mono">MINDS</span>
              </span>
            </div>
          </Link>
          ...
          <div className="hidden lg:flex items-center">
            <div className="flex items-center space-x-1 bg-black/30 backdrop-blur-md rounded-full border border-purple-500/20 p-1 mr-4">
              {navItems.map((item, index) => (
                <div key={item.name} className="relative group">
                    ...
                    <button
                      ...
                      className={`px-4 py-2 text-base font-medium rounded-full flex items-center
                        ...
                        ${activeDropdown === item.name 
                          ...
                          : 'text-white/80 hover:bg-purple-500/10'}`}
                    >
                      {item.name}
                      <ChevronDown className={`ml-1 h-4 w-4 transition-transform duration-200
                        ${activeDropdown === item.name ? 'rotate-180 text-purple-400' : ''}`} 
                      />
                    </button>
                  ) : (
                    <Link
                      ...
                      className="px-4 py-2 text-base font-medium rounded-full
                        text-white/80 hover:text-purple-400 hover:bg-purple-500/10
                  ...
                  {item.hasDropdown && (
                    <div 
                      className={`absolute top-full left-0 mt-2 w-72 bg-black/90 backdrop-blur-lg border border-purple-500/20
                        rounded-xl shadow-xl shadow-purple-900/20 overflow-hidden transition-all duration-300 origin-top-left
                        ${activeDropdown === item.name 
                      ...
                      <div className="absolute inset-0 overflow-hidden">
                        <svg width="100%" height="100%" className="absolute opacity-10">
                          <pattern id="circuitPattern" patternUnits="userSpaceOnUse" width="50" height="50" patternTransform="rotate(45)">
                            <line x1="25" y1="0" x2="25" y2="50" stroke="rgba(168, 85, 247, 0.3)" strokeWidth="0.5" />
                            <line x1="0" y1="25" x2="50" y2="25" stroke="rgba(168, 85, 247, 0.3)" strokeWidth="0.5" />
                          </pattern>
                      ...
                      <div className="p-2 relative z-10">
                        {item.items?.map((subItem, i) => (
                          <Link key={subItem.name} href={subItem.href}>
                            <div className="flex items-start space-x-3 p-3 hover:bg-purple-500/10 rounded-lg transition-colors duration-200 group">
                              <div className="p-2 mt-0.5 rounded-md bg-purple-900/40 border border-purple-500/20 group-hover:bg-purple-800/40 
                                transition-colors duration-200">
                                <subItem.icon className="h-4 w-4 text-purple-400" />
                              </div>
                              <div>
                                <div className="font-medium text-sm text-white group-hover:text-purple-400 transition-colors duration-200">
                                  {subItem.name}
                                </div>
            ...
            <Link href="/assistant" className="mr-2 relative group overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-r from-purple-600/20 to-purple-400/20 blur-md 
              ...
              >
                <div className="w-6 h-6 rounded-full flex items-center justify-center
                  bg-gradient-to-r from-purple-600 to-purple-800 shadow-inner shadow-purple-500/50">
          ...
          <div className="flex items-center space-x-3 lg:hidden">
            <Link href="/assistant">
              <Button
                variant="ghost"
                size="sm"
                className="p-1.5 text-purple-400 hover:text-purple-300 hover:bg-purple-500/10 rounded-full"
              >
            ...
            <Button
              variant="ghost"
              size="sm"
              className="p-1.5 text-white hover:text-purple-400 hover:bg-purple-500/10 rounded-full focus:ring-0"
          ...
          <div className="absolute inset-0 overflow-hidden">
            <svg width="100%" height="100%" className="absolute opacity-5">
              <pattern id="mobilecircuitPattern" patternUnits="userSpaceOnUse" width="30" height="30" patternTransform="rotate(45)">
                <line x1="15" y1="0" x2="15" y2="30" stroke="rgba(168, 85, 247, 0.5)" strokeWidth="0.5" />
                <line x1="0" y1="15" x2="30" y2="15" stroke="rgba(168, 85, 247, 0.5)" strokeWidth="0.5" />
              </pattern>
              <rect width="100%" height="100%" fill="url(#mobilecircuitPattern)" />
            </svg>
          </div>
          
          <div className="px-3 pt-3 pb-3 space-y-1 border-t border-purple-500/20 mt-3 backdrop-blur-sm relative z-10">
            {navItems.map((item) => (
                ...
                {item.hasDropdown ? (
                  <>
                    ...
                    <div className={`mt-1 space-y-1 transition-all duration-300 ease-in-out
                      ${activeDropdown === item.name ? 'max-h-96 opacity-100 pl-4 border-l border-purple-500/20 ml-3' : 'max-h-0 opacity-0 pointer-events-none overflow-hidden'}`}>
                      {item.items?.map((subItem) => (
                        <Link key={subItem.name} href={subItem.href}>
                          <div className="flex items-center space-x-2 px-3 py-3 text-sm text-gray-300 
                            hover:text-purple-400 rounded-lg transition-colors duration-200">
                            <div className="p-1 rounded-md bg-purple-900/30 border border-purple-500/20">
                              <subItem.icon className="h-3.5 w-3.5 text-purple-400" />
                            </div>
                            <div className="flex-1">
                              <div className="font-medium text-white/80">{subItem.name}</div>
                              <div className="text-xs text-purple-400/60 mt-0.5">{subItem.description}</div>
                            </div>
                          </div>
                        </Link>
                      ))}
                  ...
                  <Link href={item.href}>
                    <div className="block px-3 py-2 text-base font-medium rounded-lg border border-transparent
                      text-white hover:bg-purple-500/10 hover:text-purple-400 hover:border-purple-500/20 transition-colors duration-200">
                      {item.name}
                    </div>
                  </Link>
                )}
      ...
      <div className="absolute bottom-0 left-0 w-full h-0.5 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-purple-600/0 via-purple-600/30 to-purple-600/0"></div>
      </div>
    </nav>
  );
}